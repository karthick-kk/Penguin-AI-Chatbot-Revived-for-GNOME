...
export class ChatMessageDisplay {
    ...
    displayMessage(role, text) {
        ...
        log(`[ChatUI] displayMessage - role: "${role}", text preview: "${text ? text.substring(0, 100) + '...' : 'NULL/UNDEFINED'}"`);
        log(`[ChatUI] RAW response: "${text ? text.substring(0, 200) : 'NULL/UNDEFINED'}"`);
        log(`[ChatUI] RAW response length: ${text ? text.length : 0}`);
        ...
        log(`[ChatUI] Cleaned text preview: "${cleanedText ? cleanedText.substring(0, 100) + '...' : 'NULL/UNDEFINED'}"`);
        const formattedText = isUserMessage ? convertMD(cleanedText) : convertMD(cleanedText);
        log(`[ChatUI] formattedText preview: "${formattedText ? formattedText.substring(0, 100) + '...' : 'NULL/UNDEFINED'}"`);
    ...
    _createMessageBox(messageType, messageBoxType, text, backgroundColor, textColor, rawText) {
        ...
        // Debug: Log what markup is being set
        log(`[ChatUI] Setting label markup: "${text ? text.substring(0, 200) : 'NULL/UNDEFINED'}"`);
        log(`[ChatUI] Markup content length: ${text ? text.length : 0}`);
        log(`[ChatUI] Markup preview: "${text ? text.substring(0, 100) : 'NULL/UNDEFINED'}"`);
        let usedFallback = false;
        let usedPlaceholder = false;
        try {
            label.clutter_text.set_markup(text);
            log(`[ChatUI] set_markup succeeded.`);
            let labelText = label.clutter_text.get_text();
            log(`[ChatUI] Label text after set_markup: "${labelText ? labelText.substring(0, 100) : 'NULL/UNDEFINED'}"`);
            if ((!labelText || !labelText.trim()) && rawText && rawText.trim()) {
                // Try showing the raw response as plain text
                label.clutter_text.set_text(rawText);
                label.set_style(`background-color: orange; color: black; font-weight: bold`);
                usedPlaceholder = true;
                log(`[ChatUI] Label was empty after set_markup. Set RAW RESPONSE as fallback: "${rawText.substring(0, 100)}"`);
                ...
                usedPlaceholder = true;
                log(`[ChatUI] Label was empty after set_markup. Set placeholder.`);
            }
        } catch (e) {
            logError(e, `[ChatUI] Error setting markup. Falling back to plain text.`);
            label.clutter_text.set_text(rawText || text);
            label.set_style(`background-color: ${backgroundColor}; color: red`); // Force visible color for fallback
            usedFallback = true;
            log(`[ChatUI] set_text fallback used. Content: "${rawText ? rawText.substring(0, 100) : (text ? text.substring(0, 100) : 'NULL/UNDEFINED')}"`);
        }
        this._container.add_child(box);
        log(`[ChatUI] MessageBox added. Fallback used: ${usedFallback}, Placeholder used: ${usedPlaceholder}`);
    }
