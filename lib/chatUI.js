...
export class ChatMessageDisplay {
     ...
     */
    _scrollToBottom() {
        // Schedule the scroll to happen after UI update
        GLib.timeout_add(GLib.PRIORITY_DEFAULT, 50, () => {
            if (this._container.get_parent() && this._container.get_parent().get_parent()) {
                const adjustment = this._container.get_parent().get_parent().get_vadjustment();
                if (adjustment) {
...
export class PreferencesUI {
    ...
    constructor(schema) {
        this._schema = schema;
        this._ui = new Adw.PreferencesGroup({ title: _("Settings:") });
        this._initializeUI();
    }

    /**
     * Get the UI element
     * @returns {Adw.PreferencesGroup} - Preferences UI
     */
    get ui() {
        return this._ui;
    }
     ...
     */
    _initializeUI() {
        // This implementation moved to prefs.js
    }
}
