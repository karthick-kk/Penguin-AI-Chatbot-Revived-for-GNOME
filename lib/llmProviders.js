...
class OllamaProvider extends LLMProvider {
    ...
    _extractResponseText(response) {
        console.log(`[OllamaProvider] _extractResponseText called with response type: ${typeof response}`);
        console.log(`[OllamaProvider] Response object keys: ${Object.keys(response || {})}`);
        console.log(`[OllamaProvider] Full response: ${JSON.stringify(response, null, 2)}`);
        ...
        if (response.message && typeof response.message.content === 'string') {
            ...
            // Debug: log the raw content length and preview
            console.log(`[OllamaProvider] Raw content length: ${content.length}`);
            console.log(`[OllamaProvider] Raw content preview: ${content.substring(0, 200)}...`);
            ...
            if (thinkEndIndex !== -1) {
                ...
                if (afterThink) {
                    console.log(`[OllamaProvider] Found content after </think>: ${afterThink.substring(0, 100)}...`);
                    console.log(`[OllamaProvider] Returning extracted content: "${afterThink}"`);
                    return afterThink;
                } else {
                    console.log(`[OllamaProvider] Found </think> but no content after it`);
                }
            }
            
            // If no </think> found or no content after it, return the full content
            console.log(`[OllamaProvider] No </think> found, returning full content: "${content}"`);
            return content;
        }
        
        // fallback for other formats
        console.log(`[OllamaProvider] response.message not found or content not string`);
        if (typeof response.response === 'string') {
            console.log(`[OllamaProvider] Found response.response: "${response.response}"`);
            return response.response;
        }
        if (response.choices?.[0]?.message?.content) {
            console.log(`[OllamaProvider] Found OpenAI-style response: "${response.choices[0].message.content}"`);
            return response.choices[0].message.content;
        }
        if (response.completions?.[0]?.text) {
            console.log(`[OllamaProvider] Found completions response: "${response.completions[0].text}"`);
            return response.completions[0].text;
        }
        console.log(`[OllamaProvider] No valid content found in response, returning empty string`);
        return '';
    }
